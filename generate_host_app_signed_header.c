 ...
 *
 * Everything outside SBIT parsing (crypto, signer info, serialisation,
 * fallback path, negative overrides) is unchanged.
 */

#include "generate_host_app_signed_header.h"

/* WolfSSL — SHA-256 and Ed25519 */
#include <wolfssl/wolfcrypt/sha256.h>
#include <wolfssl/wolfcrypt/ed25519.h>

/* PolarSSL (mbedTLS) — RSA */
...
/* ===========================================================================
 * RSA (PolarSSL / mbedTLS)
 * =========================================================================*/

/** RNG callback for mbedTLS using WolfSSL's WC_RNG. */
static int polarRngCb(void *ctx, unsigned char *out, size_t len)
{
    WC_RNG *rng = (WC_RNG *)ctx;
    return (wc_RNG_GenerateBlock(rng, out, (word32)len) == 0) ? 0 : -1;
}

/**
 * Generate RSA-2048 key pair; export public modulus to pubOut (256 bytes).
 * rsaCtx is left initialised with the full key pair for subsequent signing.
 */
static ErrCodeT rsaGenKeyPair(mbedtls_rsa_context *rsaCtx,
                              ...
                              WC_RNG              *rng)
{
    ...
    rc = mbedtls_rsa_gen_key(rsaCtx, polarRngCb, rng,
                             (unsigned int)RSA_KEY_BITS, RSA_PUB_EXPONENT);
    if (rc != 0) { return ERC_CRYPTO_FAIL; }

    rc = mbedtls_mpi_write_binary(&rsaCtx->N, pubOut, HAP_SUBJECT_PUB_KEY_SIZE);
    return (rc == 0) ? ERC_NO_ERROR : ERC_CRYPTO_FAIL;
}

/**
 * PKCS#1 v1.5 sign: hash msg with WolfSSL SHA-256, sign hash with PolarSSL RSA.
 */
static ErrCodeT rsaSign(mbedtls_rsa_context *rsaCtx,
                        const uint8_t       *msg,
                        uint32_t             msgLen,
                        uint8_t              sigOut[HAP_ROOT_SIGNATURE_SIZE],
                        WC_RNG              *rng)
{
    ...
    rc = mbedtls_rsa_pkcs1_sign(rsaCtx,
                                polarRngCb, rng,
                                MBEDTLS_RSA_PRIVATE,
                                MBEDTLS_MD_SHA256,
                                HAP_MSG_DIGEST_SIZE,
 ...
 * N = public modulus (256 B), D = private exponent (256 B).
 */
                            ...
                            WC_RNG        *rng)
{
    ...
    if (mbedtls_mpi_read_binary(&ctx.N, kRootPub,  HAP_SUBJECT_PUB_KEY_SIZE) != 0 ||
        mbedtls_mpi_read_binary(&ctx.D, kRootPriv, HAP_SUBJECT_PUB_KEY_SIZE) != 0 ||
        mbedtls_mpi_lset(&ctx.E, RSA_PUB_EXPONENT) != 0)
    {
        mbedtls_rsa_free(&ctx);
        return ERC_CRYPTO_FAIL;
    }

    ctx.len = HAP_SUBJECT_PUB_KEY_SIZE;

    err = rsaSign(&ctx, msg, msgLen, sigOut, rng);
    mbedtls_rsa_free(&ctx);
    return err;
}
                                 ...
                                 NegativeTestCase_e negativeCase)
{
    ...
    if (keySize == ED25519_KEY_SIZE)
    {
        ...
        if (err != ERC_NO_ERROR) { wc_FreeRng(&rng); return err; }
    ...
    {
        err = rsaGenKeyPair(&rsaCtx, subjectPubKey, &rng);
        if (err != ERC_NO_ERROR) { wc_FreeRng(&rng); mbedtls_rsa_free(&rsaCtx); return err; }
    }
    else
    {
        wc_FreeRng(&rng);
